...
>
    ...
    @foreach ($orders as $order)
                    ...
                    <span class="inline-flex flex-wrap gap-1">
                        ...
                        @if ($canManage && in_array($order->status, ['approved', 'sent', 'partially_received', 'received', 'partially_invoiced', 'invoiced'], true))
                            <button type="button" wire:click="close({{ $order->id }})" wire:confirm="Close this purchase order?"
                                    class="rounded border border-sand px-2 py-1 text-xs font-medium text-charcoal hover:bg-sand/40">
                                Close
                            </button>
                        @endif
        ...
        @if ($amendingId === $order->id)
            <tr wire:key="order-amend-{{ $order->id }}" class="border-t border-sand/60 bg-sand/10">
                <td colspan="7" class="px-3 py-3">
                    <div class="space-y-3">
                            ...
                            <tbody>
                                @foreach ($amendLines as $index => $line)
                                    <tr wire:key="amend-line-{{ $index }}">
                                        <td class="px-1 py-1"><input type="text" wire:model="amendLines.{{ $index }}.description" class="w-full rounded border border-sand px-1.5 py-1"/></td>
                                        <td class="px-1 py-1"><input type="text" wire:model="amendLines.{{ $index }}.quantity" class="w-16 rounded border border-sand px-1.5 py-1"/></td>
                                        <td class="px-1 py-1"><input type="number" wire:model="amendLines.{{ $index }}.unit_price_ht" class="w-24 rounded border border-sand px-1.5 py-1"/></td>
                                        <td class="px-1 py-1"><input type="number" wire:model="amendLines.{{ $index }}.discount_rate_bp" class="w-16 rounded border border-sand px-1.5 py-1"/></td>
                                        <td class="px-1 py-1">
                                            <select wire:model="amendLines.{{ $index }}.expense_account_id" class="w-full rounded border border-sand px-1.5 py-1">
                                                <option value="">—</option>
                        ...
                        <div class="flex flex-wrap gap-2">
                            <button type="button" wire:click="addAmendLine" class="rounded border border-sand px-2 py-1 text-xs font-medium text-charcoal hover:bg-sand/40">
                            ...
                            <button type="button" wire:click="cancelAmend" class="rounded border border-sand px-3 py-1.5 text-xs font-medium text-charcoal hover:bg-sand/40">
                                Cancel
                            </button>
                        </div>
                    </div>
