...
<div class="min-w-0 space-y-4 print:space-y-2">
    ...
    @if ($fiscalYear === null || $state === null)
        <p class="rounded border border-border-primary bg-white p-4 text-sm text-charcoal/70">
            No fiscal year selected. Create one before running a close.
        </p>
    @else
        <div class="grid gap-4 lg:grid-cols-3">
            <div class="rounded border border-border-primary bg-white p-3 print-block">
        ...
        <div class="rounded border border-border-primary bg-white print-block">
            ...
            <table class="w-full text-sm">
                <thead>
                    <tr class="border-b border-border-primary text-left text-xs uppercase tracking-wide text-charcoal/50">
                ...
                <tbody>
                    @foreach ($state['validation']['checks'] as $check)
                        ...
                        @if ($expandedCheck === $check['code'])
                            <tr class="border-b border-border-primary/60 bg-sand/20">
                                ...
                                <div class="flex flex-wrap gap-2">
                                    ...
                                    @if ($item->status->value === 'pending')
                                        <button type="button" wire:click="startWaiver({{ $item->id }})"
                                                class="rounded border border-border-primary px-2 py-1 text-xs font-medium text-charcoal hover:border-amber-400 hover:text-amber-700">
                                            Waive
                                        </button>
                                    @endif
                                </div>
                                    ...
                                    <div class="mt-2 space-y-1">
                                        <label class="flex flex-col gap-1">
                                            ...
                                            <textarea wire:model="waiverReason" rows="2"
                                                      class="w-full rounded border border-border-primary px-2 py-1 text-xs"></textarea>
                                        </label>
                                        <div class="flex gap-2">
                                            ...
                                            <button type="button" wire:click="cancelWaiver"
                                                    class="rounded border border-border-primary px-2 py-1 text-xs text-charcoal/70">
                                                Cancel
                                            </button>
                                        </div>
                                    </div>
            ...
            <div class="space-y-3 p-3">
                ...
                <div class="grid gap-2 md:grid-cols-3 no-print">
                    <label class="flex flex-col gap-1">
                        ...
                        <input type="text" wire:model="decisionBody" class="rounded border border-border-primary px-2 py-1 text-sm">
                    </label>
                    <label class="flex flex-col gap-1">
                        <span class="text-xs text-charcoal/60">Decision date</span>
                        <input type="date" wire:model="decisionDate" class="rounded border border-border-primary px-2 py-1 text-sm">
                    </label>
                    <label class="flex flex-col gap-1">
                        <span class="text-xs text-charcoal/60">Resolution reference</span>
                        <input type="text" wire:model="resolutionReference" class="rounded border border-border-primary px-2 py-1 text-sm">
                    </label>
                </div>
