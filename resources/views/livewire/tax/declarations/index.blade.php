...
<div class="mx-auto max-w-6xl space-y-6 p-4">
    ...
    @if ($showGenerateForm)
        <form wire:submit="generate" class="space-y-3 rounded border border-border-primary bg-white p-4">
            ...
            <label class="flex flex-col gap-1">
                ...
                <select wire:model.live="genType" class="rounded border border-border-primary px-2 py-1.5 text-sm">
            ...
            @if ($genType === 'dsf_annual')
                <label class="flex flex-col gap-1">
                    ...
                    <select wire:model="genFiscalYearId" class="rounded border border-border-primary px-2 py-1.5 text-sm">
            ...
            @else
                <div class="grid gap-3 sm:grid-cols-2">
                    <label class="flex flex-col gap-1">
                        ...
                        <input type="number" wire:model="genYear" class="rounded border border-border-primary px-2 py-1.5 text-sm"/>
                    </label>
                    <label class="flex flex-col gap-1">
                        <span class="text-xs font-medium text-charcoal/70">Period month (1-12)</span>
                        <input type="number" min="1" max="12" wire:model="genMonth" class="rounded border border-border-primary px-2 py-1.5 text-sm"/>
                    </label>
                </div>
            @endif
    ...
    <div class="flex flex-wrap gap-3">
        <label class="text-sm text-charcoal/80">
            Status
            <select wire:model.live="status" class="ml-1 rounded border border-border-primary bg-white px-2 py-1 text-sm">
        ...
        <label class="text-sm text-charcoal/80">
            Type
            <select wire:model.live="type" class="ml-1 rounded border border-border-primary bg-white px-2 py-1 text-sm">
                <option value="">All</option>
