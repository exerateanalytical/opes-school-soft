...
| **I — ACCOUNT AREA, built to the screens** | DONE | 2026-08-11 | The portal's single edit form became the account area `mobile/parent-profile.png` and `mobile/account-settings.png` actually show. `Account` is now the **Parent Profile hub** (identity card, My Children with class pill + matricule, Account Information, Quick Actions, security summary); the old form is `AccountEdit`. New: `AccountSettings` (the six-row settings hub + channel On/Off chips), `NotificationSettings`, `Security` (connected devices + active sessions), `HelpSupport`. Portal is now **22 screens**. |
| **I — three things the designs show that do NOT exist** | DECISION | 2026-08-11 | Stated on screen rather than stubbed, because a control that appears to harden an account without doing so is worse than its absence. (1) **"Two-factor authentication is enabled"** — there is no 2FA anywhere (spec §1 non-goal): no endpoint, no column, no flow. Printing that sentence would tell a parent they have a protection they do not. (2) **"Change Password"** — no endpoint on this surface; the tile is absent, not dead. (3) **"Link Another Child"** — linking is `LinkGuardian`, gated on `guardians.manage`; a parent who could self-link could attach themselves to any child in the school. Rendered as a note telling them to call the office. |
| **I — what IS real on those screens** | DONE | 2026-08-11 | `Support\Portal\GuardianAccount`. **Last sign-in** from the newest `login` row in `audit_logs` (no `users.last_login_at` column exists, and adding one would duplicate a fact the audit chain already holds). **Active sessions** from Laravel's own `sessions` table, with the CURRENT one flagged — a list where every row looks equally foreign invites a parent to revoke the wrong one. **Connected devices** from Sanctum tokens named `mobile:{platform}:{device_id}`, expired ones excluded, revocable — the honest answer to "I lost my phone", since access ends now rather than in 30 days. Revocation is scoped by owner **inside** the delete, so a guessed id revokes nothing. |
| **I — route wiring gate** | DONE | 2026-08-11 | New `PortalRouteWiringTest` (5 cases): every `'portal.…'` literal in every portal view resolves to a registered route; every portal route points at a component whose view exists; and the nav, child-tab and account route sets are pinned by name. **The regex deliberately matches string LITERALS, not `route('…')` calls** — the tab strip and the bottom nav both build names in an array and call `route($var)`, so a call-anchored regex would have missed precisely the two files that link to the most screens. |
| **I — guard** | — | 2026-08-11 | Full guard **329/336**, the 3 being baseline #5, #8 and the flaky `99`. `PortalRouteWiringTest` 5/5. PHPStan over `Livewire\Portal` + `Support`: **0 errors**. NB: two concurrent pest runs against `opeschool_test_gm1` deadlocked its `migrate:fresh` mid-run — the same incident §2 warns about. Repaired with one clean `migrate:fresh`; do not run two suites against this database at once. |
...
| **G0 — PARITY HARNESS (option b)** | DONE | 2026-08-11 | `tools/design-parity/mobile/`: `serve-dist.mjs` (SPA fallback — the web export is one index.html and a plain file server 404s every route, which photographs as a broken screen), `fixture-api.mjs` (the reference designs' CONTENT through the app's real client; answers **501** on payment because the real server does), `shoot.mjs` (puppeteer-core, 426×922 @ DPR1 doubled at composite — never `--force-device-scale-factor=2`, which lays out at ~392px and lies), `screens.map.json` (all 81 slugs → routes, scroll variants as `scrollTo`), README. **Reuses the portal harness's `sheet.php`** rather than forking it; made scratch-path overridable, default unchanged. App changes needed to render at all: web target (`react-dom`, `react-native-web`, `@expo/metro-runtime`) and a **web branch in `src/storage/secure.ts`** — `expo-secure-store` has no web backend and every call threw `getValueWithKeyAsync is not a function`, so every signed-in screen rendered "Something went wrong". |
| **G0 — WHAT THE FIRST SHEET SHOWED** | — | 2026-08-11 | The harness disproved the fidelity claim on its first comparison. `my-children` built vs reference: three children in **one** card instead of three cards; **initials circles instead of photos**; **the entire per-child metric strip missing** (Academics A- / Attendance 95% / Behavior Excellent / Report Card, each with an icon); no crest; no header subtitle; no `+` action; no "Need to add another child?" CTA; no confidentiality footer; bottom nav **white instead of dark green**, **emoji instead of line icons**, wrong labels, no gold active underline. Also visible on `parent-dashboard`: **"Outstanding Fees 0 FCFA"**, which breaks the app's own stated rule 3 ("absent, not zero") — the dashboard payload carries no aggregates, so the tiles render `—` and `0` where the design shows 78% / 95% / 125,000 FCFA / 4th of 36. **The 81 screens are skeletons, not replicas.** |
| G6 re-build to the sheets | TODO | | Order matters: **chrome/nav first** (wrong on every screen simultaneously — dark green bar, line icons not emoji, gold active state, correct labels), then per-screen. Loop per screen: `expo export --platform web` → `shoot.mjs <slug>` → `sheet.php <slug>` → read the reference panel top to bottom and tick off every card, row, icon, value and footer → fix → re-shoot. A screen is done when the two panels differ only where an asset is genuinely missing (crest, illustrations). |
| G7 dashboard aggregates | TODO | | `/me/dashboard` returns children + capabilities only. The design's four tiles (academic average, attendance, outstanding fees, class rank) need real values — either extend the endpoint (server-side, capability-gated per child) or render each tile absent. Do **not** render `0`. |
