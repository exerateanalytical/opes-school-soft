...
| G4 screens | **PARTIAL — 22 of 66** | 2026-08-11 | The 81 PNGs reduce to **66 distinct screens** (11 byte-identical duplicate groups; `md5sum *.png \| sort \| uniq -w32 -d` reproduces the list). 22 are built — auth, dashboard, children, child hub/profile, results/subjects/academics, attendance, discipline, fees/balance/structure/pay/receipt/payment-history, documents, health, messages/chat/notifications/announcements, search, parent profile. **44 are not started** and are listed by name in `mobile/app/README.md`. Nothing in the foundation blocks them: each is a composition of the existing kit against an endpoint that already exists. |
| **G — NEVER RUN** | OPEN | 2026-08-11 | `npm install` has not been run and the app has **not been launched, type-checked or rendered**. No `node_modules`, no lockfile, no Expo registration. Dependency versions target Expo SDK 53 and are mutually consistent by inspection but unverified against the registry. Treat the first `npx expo start` as a debugging session, not a smoke test. |
