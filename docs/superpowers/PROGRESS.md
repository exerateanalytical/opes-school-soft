...
2b. **Never run `pest --parallel` (or `artisan test --parallel`) in this repo.** Parallel mode spins up 8 workers against `opeschool_test` simultaneously — it is rule 2's collision, self-inflicted and multiplied. On 2026-08-11 a scheduled background run used `vendor\bin\pest --parallel` and its 8 workers deadlocked an unrelated `RouteSmokeTest` run in another session (`SQLSTATE[40001]: Deadlock found ... alter table tax_declarations`). Always run the plain serial form: `DB_DATABASE=opeschool_test <php> artisan test --filter="..."`.
2c. **Before starting any test run, check for a foreign one and do not kill it.** `powershell -Command "Get-Process php -ErrorAction SilentlyContinue"` — if a suite from another session/agent is already running, WAIT or skip testing and say so in your report. Never kill a process you did not start; another session may be mid-verification and you will corrupt its results, not just your own.
...
- 2026-08-11 01:30 — Procurement detail pages enriched (supplier 114→~330, PO 103→~300, invoice 144→~430, payment 90→~290) + `procurement_detail` lang block en/fr, parity diff=0 (`17722b9`). Supplier and PO pages browser-verified against live demo data. **Invoice and payment pages NOT render-verified** — `supplier_invoices` and `supplier_payments` have ZERO rows in the demo DB, so they are compile/column-checked only. Worth seeding a demo invoice+payment so those two screens can actually be exercised.
- **Procurement schema gaps (stated on-page as unavailable, not faked):** per-line analytic split has pivot rows but no read path/labels; `is_capitalised` has only nullable ids into asset/inventory tables that rule 6 forbids importing, so no names can be shown; the scanned original `document_id` has no resolvable download route in this module.
